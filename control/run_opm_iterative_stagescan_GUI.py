...
Sheppard 08/04 - Modified to use ElveFlow fluidics control
...
from utils.data_io import (read_config_file,
                           read_fluidics_program,
                           write_metadata,
                           time_stamp,
                           append_index_filepath)
from utils.opm_setup import (setup_asi_tiger,
                             setup_obis_laser_boxx,
                             camera_hook_fn,
                             retrieve_setup_from_MM)
...
def main():
    ...
    run_fluidics = False
    flush_system = False
    run_type = easygui.choicebox('Type of run?',
                                 'OPM setup',
                                 ['Run fluidics (no imaging)',
                                  'Iterative imaging',
                                  'Single round (test)'])
    ...
    if run_fluidics:
        # Open ElveFlow connection and start remote operation
        flow_control = FlowControl()
        flow_control.startup()
        ...
        print(f"Will execute fluidics rounds: {fluidics_rounds}",
              f"Number of iterative rounds: {str(n_iterative_rounds)}")

        if max_round_name != n_iterative_rounds:
            print(f"Max round label is {max_round_name}")

    if flush_system:
        # Run fluidics system flush
        success_fluidics = flow_control.run_flush()
    ...
    # set ROI
    roi_selection = easygui.choicebox("Imaging volume setup.",
                                      "ROI size",
                                      ["256x1900", "512x1900", "1024x1900"])
    ...
    resume_r_name = -1     # round index in human notation, starts from 1
    resume_y_tile_idx = -1
    resume_z_tile_idx = -1
    ...
    for r_idx, r_name in enumerate(fluidics_rounds):
        ...
        if run_fluidics:
            ...
            success_fluidics = run_fluidic_program(r_name,
                                                   df_program,
                                                   flow_control)
            if not(success_fluidics):
                print("Error in fluidics! Stopping scan.")
            ...
            O3_focus_positions=manage_O3_focus(core,shutter_controller,
                                               O3_stage_name,
                                               verbose=False)
            print(time_stamp(), f"O3 focus stage position (um) = {O3_focus_positions}.")
        ...
        if switch_last_round and (r_idx == (n_iterative_rounds - 1)) and (run_fluidics):
            ...
            df_MM_setup, active_channel_indices = retrieve_setup_from_MM(core,
                                                                         studio,
                                                                         df_config,
                                                                         debug=debug_flag)
        ...
        if config_changed:
            ...
            core.set_xy_position(float(df_MM_setup['scan_axis_start_um']),
                                 float(df_MM_setup['tile_axis_start_um']))
            core.wait_for_device(xy_stage)

            # setup constant speed stage scanning on ASI Tiger controller
            setup_asi_tiger(core,
                            float(df_MM_setup['scan_axis_speed']),
                            float(df_MM_setup['scan_axis_start_mm']),
                            float(df_MM_setup['scan_axis_end_mm']))
            ...
            for excess_scan_idx in range(int(df_config['excess_scan_positions'])):
                for c in active_channel_indices:
                    evt = { 'axes': {'s': 0,
                                     'channel': str(channel_names[int(c)]),
                                     'e': excess_scan_idx}}
                    events.append(evt)
            for scan_idx in range(int(df_MM_setup['scan_axis_positions'])):
                for c in active_channel_indices:
                    evt = { 'axes': {'s': scan_idx,
                                     'channel': str(channel_names[int(c)]),
                                     'e': (int(df_config['excess_scan_positions'])+1)}}
                    events.append(evt)
        ...
        for y_idx in range(resume_y_tile_idx,n_y_tiles):
            ...
            for z_idx in range(resume_z_tile_idx,n_z_tiles):
                ...
                if (r_name == resume_r_name) and (y_idx == resume_y_tile_idx) and (z_idx == resume_z_tile_idx):
                    save_name_ryz = Path(str(df_MM_setup['save_name'])+
                                         '_r'+str(r_name).zfill(4)+
                                         '_y'+str(y_idx).zfill(4)+
                                         '_z'+str(z_idx).zfill(4)+'_a')
                else:
                    save_name_ryz = Path(str(df_MM_setup['save_name'])+
                                         '_r'+str(r_name).zfill(4)+
                                         '_y'+str(y_idx).zfill(4)+
                                         '_z'+str(z_idx).zfill(4))
                ...
                # run O3 focus optimizer
                O3_focus_position = manage_O3_focus(core,shutter_controller,
                                                    O3_stage_name,
                                                    verbose=False)
                ...
                if z_idx > 1:
                    ...
                    offset_O3_focus_position = apply_O3_focus_offset(core,
                                                                     O3_stage_name,
                                                                     O3_focus_position,
                                                                     O3_focus_offset,
                                                                     verbose=False)
                ...
                try:
                    ...
                    taskDO.WriteDigitalLines(samples_per_ch,
                                             False, 10.0,
                ...
                if (r_name == resume_r_name) and (y_idx == resume_y_tile_idx) and (z_idx == resume_z_tile_idx):
                    save_name_stage_positions = Path(str(df_MM_setup['save_name'])+
                                                     '_r'+str(r_name).zfill(4)+
                                                     '_y'+str(y_idx).zfill(4)+
                                                     '_z'+str(z_idx).zfill(4)+
                                                     '_a_stage_positions.csv')
                else:
                    save_name_stage_positions = Path(str(df_MM_setup['save_name'])+
                                                     '_r'+str(r_name).zfill(4)+
                                                     '_y'+str(y_idx).zfill(4)+
