'''
Initial work on napari interface using headless MM, pycro-manager, and magic-class
'''
...
def main():
    ...
    # take one 3D volume (hardware triggering during sweep)
    @magicgui(
        auto_call=True,
        start_3D_once={"widget_type": "PushButton", "label": 'Acquire once (3D)'},
        layout='horizontal'
    )
    def acquire_3D_once(start_3D_once):
        global worker_started
        global scan_finished
    ...
    viewer.window.add_dock_widget([acquire_3D_once,start_live_mode_3D,stop_live_mode_3D],name='Live mode')
    napari.run()
